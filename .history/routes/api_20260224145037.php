...
Route::middleware(['auth:sanctum', 'throttle:60,1'])->group(function () {
    ...
    Route::prefix('api-keys')->group(function () {
        Route::get('/', [ApiKeyController::class, 'index'])->name('api.api-keys.index');
        // other operations (show, update, usage) remain available for external API use
        Route::get('{apiKey}', [ApiKeyController::class, 'show'])->name('api.api-keys.show');
        Route::put('{apiKey}', [ApiKeyController::class, 'update'])->name('api.api-keys.update');
        Route::delete('{apiKey}', [ApiKeyController::class, 'destroy'])->name('api.api-keys.destroy');
        Route::post('{apiKey}/regenerate', [ApiKeyController::class, 'regenerate'])->name('api.api-keys.regenerate');
        Route::get('{apiKey}/usage', [ApiKeyController::class, 'usage'])->name('api.api-keys.usage');
    });
});

// Ollama Proxy API - Protected with authentication and usage tracking
Route::prefix('ollama')->middleware(['auth:sanctum', 'throttle:120,1', 'track.api.usage'])->group(function () {
    // Chat & Generation
    ...
    // Embeddings
    Route::post('embed', [OllamaProxyController::class, 'embed'])->name('api.ollama.embed');
});
