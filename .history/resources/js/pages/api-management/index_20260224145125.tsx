...
import AppLayout, { AppLayout as NamedAppLayout } from '@/layouts/app-layout';
...
import { Key, TrendingUp, Activity, Clock, Plus, Copy, Trash2, RefreshCw, EyeOff } from 'lucide-react';
...
export default function ApiManagement({
    ...
    const handleCreateApiKey = async () => {
        try {
            ...
            const { api_key, plain_text_token, message } = response.data;

            setNewTokenVisible(plain_text_token);
            toast.success(message);

            // add new key to local list so it shows immediately
            setApiKeys((prev) => [...prev, api_key]);
    ...
    const handleRegenerateApiKey = async (keyId: number) => {
        ...
        try {
            const response = await axios.post(`/api-keys/${keyId}/regenerate`);
            const { api_key, plain_text_token, message } = response.data;
            setNewTokenVisible(plain_text_token);
            toast.success(message);
            setApiKeys((prev) => prev.map((k) => (k.id === api_key.id ? api_key : k)));
        } catch (error: any) {
                                ...
                                {recentActivity.slice(0, 20).map((activity) => (
                                    <TableRow key={activity.id}>
                                        ...
                                        <TableCell>
                                            <Badge
                                                variant={
                                                    activity.is_successful ? 'default' : 'destructive'
                                                }
                                            >
